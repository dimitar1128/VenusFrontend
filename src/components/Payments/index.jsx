import React from 'react';
import ReactTable from 'react-table-v6';
import { Row, Col, Card, Form, Button, Modal } from 'react-bootstrap';

import Toaster from '../../App/layout/Toast/index';
import Modaler from '../../App/layout/Modal/index';
import request from '../../services/request';
import Aux from '../../hoc/_Aux';
import { table_columns, getCurrentDate, getProjectNameById } from './utils';

import 'react-table-v6/react-table.css';
import './index.css';

const formInitValues = {
  date: getCurrentDate(),
  for_plan: '1',
  is_directly: '1',
};

class Payments extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
      formValues: {},
      projects: [],
      toastVisible: false,
      toastMessage: '',
      modalarVisible: false,
      modalarMessage: 'Are you sure you want to delete this project ?',
    };
  }

  async componentDidMount() {
    try {
      const projects = await request.get('project/');
      const result = await request.get('payment/');
      this.setState({ projects: projects.data, data: result.data });
    } catch (error) {
      console.log(error);
    }
  }

  deleteItem = (row) => async () => {
    this.setState({ modalarVisible: true, deleteId: row.original.id });
  };

  editItem = (row) => () => {
    const newData = JSON.parse(JSON.stringify(this.state.data));
    this.setState({
      data: newData,
      selectedEle: true,
      formValues: row.original,
      showModal: true,
    });
  };

  close = async () => {
    const { formValues, selectedEle, projects } = this.state;
    const newData = JSON.parse(JSON.stringify(this.state.data));
    if (!selectedEle) {
      const result = await request.post('payment/', formValues);
      const { id, project_id } = result.data;
      newData.push({
        ...formValues,
        id,
        project_name: getProjectNameById(project_id, projects),
      });
    } else {
      await request.put(`payment/${formValues.id}/`, formValues);
      const exist = newData.find((x) => x.id === formValues.id);
      const index = newData.findIndex((x) => x.id === formValues.id);
      if (index >= 0) {
        newData[index] = {
          ...exist,
          ...formValues,
          is_directly: +formValues.is_directly,
          for_plan: +formValues.for_plan,
          project_name: getProjectNameById(+formValues.project_id, projects),
        };
      }
    }
    const toastMessage = selectedEle
      ? 'Payment has been successfully updated.'
      : 'Payment has been successfully added.';
    this.setState({
      showModal: false,
      data: newData,
      toastVisible: true,
      toastMessage,
    });
  };

  open = () => {
    this.setState({
      selectedEle: null,
      formValues: formInitValues,
      showModal: true,
    });
  };

  textChange = (e) => {
    const { formValues } = this.state;
    const { name, value } = e.target;
    this.setState({
      formValues: { ...formValues, [name]: value },
    });
  };

  toastChange = () => {
    const { toastVisible } = this.state;
    this.setState({ toastVisible: !toastVisible });
  };

  modalarChange = () => {
    const { modalarVisible } = this.state;
    this.setState({ modalarVisible: !modalarVisible });
  };

  yesAction = async () => {
    this.modalarChange();
    const { deleteId } = this.state;
    try {
      const newData = JSON.parse(JSON.stringify(this.state.data));
      await request.delete(`payment/${deleteId}`);
      this.setState({
        data: newData.filter((x) => x.id !== deleteId),
        toastVisible: true,
        toastMessage: 'Payment has been successfully deleted.',
      });
    } catch (error) {
      console.log(error);
    }
  };
  render() {
    const columns = [
      ...table_columns,

      {
        Header: 'Actions',
        accessor: 'name',
        Cell: (row) => (
          <React.Fragment>
            <div className="text-center">
              <i
                className="feather icon-edit"
                style={{ fontSize: '18px' }}
                onClick={this.editItem(row)}
              />
              <i
                className="feather icon-trash"
                style={{ fontSize: '18px' }}
                onClick={this.deleteItem(row)}
              />
            </div>
          </React.Fragment>
        ),
      },
    ];

    return (
      <Aux>
        <Row>
          <Toaster
            isVisible={this.state.toastVisible}
            onChange={this.toastChange}
            message={this.state.toastMessage}
          />
          <Modaler
            isVisible={this.state.modalarVisible}
            onChange={this.modalarChange}
            message={this.state.modalarMessage}
            yesAction={this.yesAction}
          />
          <Col md={12} xl={12}>
            <Card className="card-social">
              <Card.Body>
                <Button
                  bsstyle="primary"
                  bssize="large"
                  onClick={this.open}
                  className="mb-3"
                >
                  Add Payments
                </Button>

                <Modal
                  show={this.state.showModal}
                  onHide={() => this.setState({ showModal: false })}
                >
                  <Modal.Header closeButton>
                    <Modal.Title>
                      {this.state.selectedEle ? 'Edit ' : 'Add'} Payment
                    </Modal.Title>
                  </Modal.Header>
                  <Modal.Body>
                    <Row>
                      <Col md={12}>
                        <Form>
                          <Form.Group as={Row}>
                            <Form.Label column sm="3">
                              Project
                            </Form.Label>
                            <Col sm="9">
                              <Form.Control
                                name="project_id"
                                as="select"
                                className="mb-3"
                                value={this.state.formValues?.project_id}
                                onChange={this.textChange}
                              >
                                {[
                                  { id: 0, title: '' },
                                  ...this.state.projects,
                                ].map((x) => (
                                  <option key={x.id} value={x.id}>
                                    {x.name}
                                  </option>
                                ))}
                              </Form.Control>
                            </Col>
                          </Form.Group>
                          <Form.Group as={Row}>
                            <Form.Label column sm="3">
                              Amount
                            </Form.Label>
                            <Col sm="9">
                              <Form.Control
                                name="amount"
                                type="number"
                                value={this.state.formValues?.amount}
                                onChange={this.textChange}
                              />
                            </Col>
                          </Form.Group>

                          <Form.Group as={Row}>
                            <Form.Label column sm="3">
                              Date
                            </Form.Label>
                            <Col sm="9">
                              <Form.Control
                                name="date"
                                type="date"
                                value={this.state.formValues?.date}
                                onChange={this.textChange}
                              />
                            </Col>
                          </Form.Group>

                          <Form.Group as={Row}>
                            <Form.Label column sm="3">
                              For plan
                            </Form.Label>
                            <Col sm="9">
                              <Form.Control
                                name="for_plan"
                                as="select"
                                className="mb-3"
                                value={this.state.formValues?.for_plan}
                                onChange={this.textChange}
                              >
                                <option value="1">Yes</option>
                                <option value="0">No</option>
                              </Form.Control>
                            </Col>
                          </Form.Group>

                          <Form.Group as={Row}>
                            <Form.Label column sm="3">
                              Directly
                            </Form.Label>
                            <Col sm="9">
                              <Form.Control
                                name="is_directly"
                                as="select"
                                className="mb-3"
                                value={this.state.formValues?.is_directly}
                                onChange={this.textChange}
                              >
                                <option value="1">Yes</option>
                                <option value="0">No</option>
                              </Form.Control>
                            </Col>
                          </Form.Group>

                          <Form.Group as={Row}>
                            <Form.Label column sm="3">
                              Note
                            </Form.Label>
                            <Col sm="9">
                              <Form.Control
                                name="note"
                                as="textarea"
                                rows="3"
                                value={this.state.formValues?.note}
                                onChange={this.textChange}
                              />
                            </Col>
                          </Form.Group>
                        </Form>
                      </Col>
                    </Row>
                  </Modal.Body>
                  <Modal.Footer>
                    <Button onClick={this.close}>
                      {this.state.selectedEle ? 'Update' : 'Submit'}
                    </Button>
                  </Modal.Footer>
                </Modal>
                <ReactTable
                  data={this.state.data}
                  columns={columns}
                  defaultPageSize={15}
                  pageSizeOptions={[5, 10, 15, 20, 25, 50, 100]}
                  className="react-table"
                />
              </Card.Body>
            </Card>
          </Col>
        </Row>
      </Aux>
    );
  }
}

export default Payments;
